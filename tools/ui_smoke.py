...
class RealControls(unittest.TestCase):
    def runTest(self):
        ...
        try:
            ...
            self.assertEqual(panel.output.GetString(0), "Test speakers")
            ...
            self.assertEqual(panel.output.GetString(1), "Second test output")
            self.assertFalse(panel.scanTimer.IsRunning())
            panel.volume.SetValue(11)
            panel.enableBox.SetValue(True)
            panel.mode.SetSelection(3)  # Several selected outputs.
            panel.onMode(None)
            panel.outputs.Check(0)
            panel.outputs.Check(1)
            panel.onSave()
            self.assertEqual(fixture.plugin.store.data["volume"], 11)
            self.assertEqual(fixture.plugin.store.data["mode"], "devices")
            self.assertEqual(fixture.plugin.store.data["devices"], ["one", "two"])
            panel.onStop(None)
